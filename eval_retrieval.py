...
class CLIController(CLI):
    ...
    def run(self):
        ...
        train = dataset['train']
        results = storage.batch_query(self._args.collection, train['question'],
                                     self._args.number_results)


        precisions = np.zeros((len(train),), dtype=np.float32)
        recalls = np.zeros((len(train),), dtype=np.float32)
        f1s = np.zeros((len(train),), dtype=np.float32)
        for q, question in enumerate(train):
            n_relevant = 0
            all_relevant = storage.get_all_from_parent(self._args.collection, question['title'],
                                                       question['context'])
            for document in results[q]:
                if self.__is_relevant(question, document):
                    n_relevant += 1

            precision = n_relevant/self._args.number_results
        ...
        print("N Samples:", len(train))
        print(f"Precision@{self._args.number_results}:{precisions.mean()}")
        print(f"Recall@{self._args.number_results}:{recalls.mean()}")
        print(f"F1@{self._args.number_results}:{f1s.mean()}")
